  ...
  return (
    ...
    >
      ...
      <SidebarContent className="w-full flex flex-col gap-6 p-0 scrollbar-none">
        <CustomSidebarGroup title="Menu" data={menu_links} />
  ...
  return (
    <SidebarGroup title={title}>
      <div className="flex items-center justify-between">
        ...
        {action && (
          <SidebarGroupAction title="New project" onClick={action}>
    ...
    <SidebarMenuItem title={title}>
      <SidebarMenuButton
        ...
        className={cn(
          ...
          isActive
            ? "font-bold text-primary-600 bg-grey-50 hover:bg-grey-50"
            : inActiveClassName
        )}
      >
        <Link href={href}>
          {Icon ? (
            ...
            <div className="size-3 bg-[#6366F1] rounded-full" />
          )}
          <span>{title}</span>
        </Link>
      </SidebarMenuButton>
...
const menu_links: IMenuLinkProps = {
  inActiveClassName:
    "font-medium tracking-[0.2px] text-grey-500 bg-transparent",
  links: [
    {
      title: "Dashboard",
...
const projects: IMenuLinkProps = {
  inActiveClassName:
    "font-semibold text-grey-900 tracking-[0.2px] bg-transparent",
  links: [
